...
class SelectRecipients(QDialog):  # {{{
    ...
    def __init__(self, parent=None):
        ...
        self.add_button = b = QPushButton(
            QIcon(I('plus.png')), _('&Add recipient'), self)
    ...
    def add_recipient(self):
        ...
        formats = ','.join([x.strip().upper() for x in str(
            self.formats.text()).strip().split(',') if x.strip()])
        if not formats:
            return error_dialog(
    ...
    def ans(self):
        ...
        for i in self.items:
            if i.checkState() == Qt.CheckState.Checked:
                ...
                fmts = tuple(x.strip().upper()
                             for x in (opts.accounts[to][0] or '').split(','))
...
class EmailMixin:  # {{{
    ...
    def send_multiple_by_mail(self, recipients, delete_from_library):
        ...
        if not ids:
            return
        db = self.current_db
        db_fmt_map = {book_id: set((db.formats(
            book_id, index_is_id=True) or '').upper().split(',')) for book_id in ids}
        ...
        for to, fmts, subject in recipients:
            rfmts = set(fmts)
            ok_ids = {book_id for book_id, bfmts in iteritems(
                db_fmt_map) if bfmts.intersection(rfmts)}
            convert_ids = ids - ok_ids
            self.send_by_mail(to, fmts, delete_from_library,
                              subject=subject, send_ids=ok_ids, do_auto_convert=False)
            if not rfmts.intersection(ofmts):
                bad_recipients[to] = (convert_ids, True)
                continue
            outfmt = tuple(f for f in fmts if f in ofmts)[0]
            ok_ids = {
                book_id for book_id in convert_ids if db_fmt_map[book_id].intersection(ifmts)}
        ...
        if auto_convert_map:
            titles = {book_id for x in itervalues(
                auto_convert_map) for data in x for book_id in data[2]}
            titles = {db.title(book_id, index_is_id=True)
                      for book_id in titles}
            if self.auto_convert_question(
                ...
                for ofmt, data in iteritems(auto_convert_map):
                    ...
                    self.iactions['Convert Books'].auto_convert_multiple_mail(
                        ids, data, ofmt, delete_from_library)

        if bad_recipients:
            det_msg = []
            titles = {book_id for x in itervalues(
                bad_recipients) for book_id in x[0]}
            titles = {book_id: db.title(book_id, index_is_id=True)
                      for book_id in titles}
    ...
    def send_by_mail(self, to, fmts, delete_from_library, subject='', send_ids=None,
        ...
        ids = [self.library_view.model().id(r) for r in self.library_view.selectionModel(
        ).selectedRows()] if send_ids is None else send_ids
        ...
        for f, mi, id, newmi in zip(files, full_metadata, ids, modified_metadata):
            ...
            if f is None:
                ...
                if mi.comments and gprefs['add_comments_to_email']:
                    ...
                    if mi.series:
                        sidx = fmt_sidx(1.0 if mi.series_index is None else mi.series_index,
                                        use_roman=config['use_roman_numerals_for_series_number'])
                        texts[-1] += '\n\n' + _('{series_index} of {series}').format(
                            series_index=sidx, series=mi.series)
                    texts[-1] += '\n\n' + _('About this book:') + \
        ...
        if auto != []:
            format = specific_format if specific_format in list(
                set(fmts).intersection(set(available_output_formats()))) else None
            if not format:
                for fmt in fmts:
            ...
            if format is None:
                ...
                autos = [self.library_view.model().db.title(
                    id, index_is_id=True) for id in auto]
                if self.auto_convert_question(
